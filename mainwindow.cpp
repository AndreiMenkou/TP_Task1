...
void MainWindow::createCentralWidget()
{
    ...
    graphicsView->setMinimumSize(600, 500);
...
//    fillColorAction->setEnabled(false);
    ...
    borderColorAction = new QAction(createColorToolButtonIcon(scene.getBorderColor()), "Border color", this);
    colorBar->addAction(borderColorAction);
    connect(borderColorAction, SIGNAL(triggered()), this, SLOT(borderColorChanged()));
}
...
void MainWindow::borderColorChanged() {
    scene.setBorderColor(QColorDialog::getColor(scene.getBorderColor()));
    borderColorAction->setIcon(createColorToolButtonIcon(scene.getBorderColor()));
}
